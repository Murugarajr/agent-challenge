...
class TightCouplingDetector:
    ...
    def _generate_global_di_example(self, func_name: str, globals_used: Set[str]) -> str:
        ...
        # Pre-compute strings that require a backslash escape — Python < 3.12
        # does not allow backslash characters inside f-string {} expressions.
        _nl = '\n        '
        _init_body = ''.join(
            f'self.{g}_service = {g}_service{_nl}' for g in globals_list
        )
        _params = ', '.join(f'{g}_service' for g in globals_list)
        _kwargs = ', '.join(f'{g}_service=service' for g in globals_list)
