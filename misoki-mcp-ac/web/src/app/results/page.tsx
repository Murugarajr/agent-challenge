...
import type { AnalysisIssue, ApplyFixResponse, CreatePrResponse, PreviewFixResponse, RepoAnalyzeResponse } from "@/lib/types";
import { createDraftPr, previewFix } from "@/lib/api";
...
export default function ResultsPage() {
    ...
    function handlePatched(result: ApplyFixResponse) {
        ...
        setPrState({ loading: false, result: null, error: null });
    }

    async function handleCreatePr() {
        if (!analysis || !applyState.result?.files.length) return;
        setPrState({ loading: true, result: null, error: null });
        const repoName = analysis.repo.replace(/\/$/, "").split("/").pop() ?? "repo";
        const title = `chore(${repoName}): apply Misoki safe fixes`;
        const body = [
            "## Summary",
            "",
            `Automated safe-fix patches generated by [Misoki](${analysis.repo}) analysis.`,
            "",
            `- **Repo analyzed:** ${analysis.repo}`,
            `- **Commit:** ${analysis.commit}`,
            `- **Patches applied:** ${applyState.result.applied_count}`,
            `- **Skipped:** ${applyState.result.skipped_count}`,
            `- **Files changed:** ${applyState.result.files.map(f => f.file).join(", ")}`,
            "",
            "## Applied fixes",
            "",
            ...applyState.result.applied_fix_ids.map(id => `- \`${id}\``),
            "",
            "---",
            "*Generated by Misoki — Code Quality Analysis, Supercharged by AI*",
        ].join("\n");
        ...
        try {
            const result = await createDraftPr(
                analysis.repo,
                analysis.branch,
                applyState.result.files,
                title,
                body,
            );
            setPrState({ loading: false, result, error: null });
        } catch (err) {
            setPrState({ loading: false, result: null, error: err instanceof Error ? err.message : String(err) });
        }
    }
                ...
                >
                    ...
                    >
                        <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12, background: "var(--surface-1)" }}>
                            ...
                            {applyState.result.files.length > 0 && !prState.result && (
                                <button
                                    onClick={handleCreatePr}
                                    disabled={prState.loading}
                                    style={{
                                        background: prState.loading ? "var(--surface-3)" : "linear-gradient(135deg, #7c3aed, #9333ea)",
                                        color: prState.loading ? "var(--text-muted)" : "#fff",
                                        border: "none", borderRadius: 6, cursor: prState.loading ? "not-allowed" : "pointer",
                            ...
                            {prState.result && (
                                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                    ...
                                    >
                                        ...
                                        View PR #{prState.result.pr_number}
                                    </a>
                                    {prState.result.forked && (
                                        <span style={{ fontSize: "0.72rem", color: "var(--accent-purple)", background: "rgba(139,92,246,0.12)", padding: "2px 8px", borderRadius: 4 }}>
                                            via fork: {prState.result.work_repo}
                                        </span>
                                    )}
                                </div>
                            )}
